        ...
        <div className="w-full max-w-4xl mx-auto mb-8">
          <div className="clean-card p-6 bg-gradient-to-br from-blue-50 to-orange-50 dark:from-blue-900/20 dark:to-orange-900/20 border border-primary-border dark:border-primary-border-dark shadow-lg flex flex-col md:flex-row items-center gap-6 animate-fade-in-up">
            <div className="flex-shrink-0">
              <svg className="w-20 h-20 md:w-24 md:h-24" viewBox="0 0 64 64" fill="none">
                <circle cx="32" cy="32" r="32" fill="#fb923c" fillOpacity="0.12" />
                <path d="M24 32h16M32 24v16" stroke="#3b82f6" strokeWidth="3" strokeLinecap="round" />
                <rect x="28" y="28" width="8" height="8" rx="2" fill="#3b82f6" />
              </svg>
            </div>
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-2 text-primary-text dark:text-primary">Task Suggestion for You</h2>
              <div className="mb-2 text-lg font-semibold text-blue-700 dark:text-blue-300">{availableTasks[0].title}</div>
              <div className="mb-2 text-secondary-text dark:text-secondary-text-dark">{availableTasks[0].description}</div>
              <div className="flex flex-wrap gap-2 mb-4">
                {availableTasks[0].skillsRequired.map((skill, idx) => (
                  <span key={idx} className="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-xs font-medium border border-blue-200 dark:border-blue-700">{skill}</span>
                ))}
              </div>
              <button
                className={`px-5 py-2 btn-primary rounded-xl text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300 bg-primary-main ${appliedTaskIds.includes(availableTasks[0].id) ? 'opacity-60 cursor-not-allowed' : ''}`}
                disabled={appliedTaskIds.includes(availableTasks[0].id)}
                onClick={() => handleApply(availableTasks[0].id)}
              >
                {appliedTaskIds.includes(availableTasks[0].id) ? 'Applied' : 'Apply Instantly'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="w-full px-6 py-8">
        <div className="w-full max-w-4xl mx-auto mb-8">
          <div className="clean-card p-6 bg-gradient-to-br from-blue-50 to-orange-50 dark:from-blue-900/20 dark:to-orange-900/20 border border-primary-border dark:border-primary-border-dark shadow-lg flex flex-col md:flex-row items-center gap-6 animate-fade-in-up">
            <div className="flex-shrink-0">
              <svg className="w-20 h-20 md:w-24 md:h-24" viewBox="0 0 64 64" fill="none">
                <circle cx="32" cy="32" r="32" fill="#3b82f6" fillOpacity="0.12" />
                <path d="M32 16v20" stroke="#3b82f6" strokeWidth="3" strokeLinecap="round" />
                <circle cx="32" cy="44" r="2.5" fill="#fb923c" />
                <rect x="28" y="36" width="8" height="4" rx="2" fill="#fb923c" />
              </svg>
            </div>
            <div>
              <h2 className="text-2xl font-bold mb-2 text-primary-text dark:text-primary">Quick Tips for Providers</h2>
              <ul className="list-disc pl-5 space-y-1 text-secondary-text dark:text-secondary-text-dark text-base">
                ...
                <li>Keep your profile updated and showcase your best skills.</li>
                <li>Communicate clearly and promptly with clients.</li>
                <li>Deliver quality work to earn great reviews and more tasks.</li>
